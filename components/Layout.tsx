  ...
  return (
    ...
    >
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-gray-200 p-6 shadow-[2px_0_24px_-12px_rgba(0,0,0,0.05)] z-30 relative shrink-0 h-full">
        ...
        {miniPlayer && (
            <div className="mb-6 animate-fade-in-down shrink-0">
                {miniPlayer}
            </div>
        )}
        ...
        {currentView !== 'routine-player' && (
            <div className="mt-auto pt-6 border-t border-gray-100 shrink-0" style={{ zoom: 1 / uiScale }}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-gray-50">
        {/* Conditional container: Full width/height for calendar/notes/projects/habits, padded scrollable for others */}
        <div className={`flex-1 flex flex-col h-full ${isFullWidthView ? 'overflow-hidden p-0' : 'overflow-y-auto p-4 md:p-6 pb-24 md:pb-12 custom-scrollbar'}`}>
          {children}
        </div>
      </main>
