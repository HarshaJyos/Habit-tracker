...
import { 
  Pause, Play, X, Check, FastForward, CheckSquare, Clock, 
  GripVertical, ArrowDown, PauseCircle, CheckCircle2, Maximize2, 
  SkipForward, PanelLeft, Plus, Minus, Search, GripHorizontal, 
  MoreHorizontal, ChevronDown, ChevronUp, Trash2, List,
  Layers, PlusCircle, Minimize2
} from 'lucide-react';
...
export const RoutinePlayer: React.FC<RoutinePlayerProps> = ({ 
  ...
  // Mobile state for "Up Next" drawer
  ...
  React.useEffect(() => {
      const calculateEstimation = () => {
          ...
          // Add duration of all future steps
      ...
      calculateEstimation();
      const interval = setInterval(calculateEstimation, 1000); // Update every minute/second to keep clock accurate
  ...
  const handleDragStart = (e: React.DragEvent, index: number, origin: 'list' | 'library', id?: string, type?: string) => {
          ...
          if (type === 'task') {
              ...
              if (habit) {
                  newStep = {
                      id: `${Date.now()}-${Math.random()}`,
                      title: habit.title,
                      durationSeconds: habit.goal.type === 'duration' ? habit.goal.target * 60 : 300,
    ...
    <div className="fixed inset-0 bg-zinc-950 text-white z-[100] flex flex-col font-sans h-full w-full overflow-hidden">
      ...
      <div className="h-16 border-b border-zinc-800 flex items-center justify-between px-4 md:px-6 shrink-0 bg-zinc-950 z-20">
         <div className="flex items-center gap-3">
             ...
             <span className="text-sm font-bold tracking-widest uppercase text-zinc-300 hidden md:block">Focus Session</span>
             {/* Mobile Title (Truncated) */}
             <span className="text-sm font-bold tracking-tight text-white md:hidden truncate max-w-[120px]">{currentStep.title}</span>
         </div>
         <div className="flex gap-2">
            ...
            <button onClick={onSave} className="flex items-center gap-2 text-zinc-400 hover:text-white px-3 py-1.5 hover:bg-zinc-800 rounded-lg transition-colors text-sm font-medium" title="Save & Quit">
                <PauseCircle size={18} /> <span className="hidden md:inline">Save & Quit</span>
            </button>
            <button onClick={onExit} className="flex items-center gap-2 text-zinc-400 hover:text-red-400 px-3 py-1.5 hover:bg-zinc-800 rounded-lg transition-colors text-sm font-medium" title="Cancel">
      ...
      <div className="flex-1 flex overflow-hidden relative">
          ...
          {/* LEFT PANEL: Library & Trash (Desktop Only) */}
          <div className="hidden lg:flex flex-col w-72 border-r border-zinc-800 bg-zinc-950/50">
              <div className="p-4 border-b border-zinc-800">
                  <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-4">Library</h3>
                  
                  {/* Remove Drop Zone */}
                  <div 
                    className={`border-2 border-dashed rounded-xl p-4 text-center transition-colors ${isRemoveZoneActive ? 'border-red-500 bg-red-500/10 text-red-500' : 'border-zinc-800 text-zinc-600'}`}
                    onDragOver={handleRemoveZoneDragOver}
                    onDragLeave={handleRemoveZoneDragLeave}
                    onDrop={handleRemoveDrop}
                  >
                      <Trash2 size={20} className="mx-auto mb-2" />
                      <span className="text-[10px] font-bold uppercase tracking-wider">Drag steps here to remove</span>
                  </div>
              </div>
                      ...
                      <div className="space-y-2">
                          {tasks.filter(t => !t.isCompleted).map(task => (
                              <div 
                                  key={task.id} 
                                  draggable 
                                  onDragStart={(e) => handleDragStart(e, -1, 'library', task.id, 'task')}
                      ...
                      <div className="space-y-2">
                          {habits.map(habit => (
                              <div 
                                  key={habit.id} 
                                  draggable 
                                  onDragStart={(e) => handleDragStart(e, -1, 'library', habit.id, 'habit')}
          ...
          <div className="flex-1 flex flex-col relative bg-zinc-950 overflow-y-auto custom-scrollbar">
               {/* Mobile "Up Next" Toggler */}
               <div className="lg:hidden absolute top-4 right-4 z-20">
                   <button onClick={() => setIsMobileSequenceOpen(true)} className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 px-3 py-2 rounded-full text-xs font-bold text-zinc-400">
               ...
               <div className="flex-1 flex flex-col items-center justify-center p-6 min-h-[600px] lg:min-h-0 w-full max-w-md mx-auto">
                   ...
                   <div className="text-center mb-8 shrink-0 w-full">
                       <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-3">{routine.title}</h3>
                   ...
                   <div className="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 group mb-10 shrink-0">
                       <svg className="w-full h-full transform -rotate-90 drop-shadow-2xl">
                           <circle cx="50%" cy="50%" r="46%" stroke="#18181b" strokeWidth="8" fill="transparent" />
                           <circle cx="50%" cy="50%" r="46%" stroke={isOvertime ? "#ef4444" : "#10b981"} strokeWidth="8" fill="transparent" 
                       ...
                       <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                           <span className={`text-6xl md:text-7xl lg:text-8xl font-mono font-bold tracking-tighter tabular-nums ${isOvertime ? 'text-red-500 animate-pulse' : 'text-white'}`}>
                               {formatTime(timeLeft)}
                           </span>
                           {isOvertime && <span className="text-xs font-bold text-red-500 uppercase tracking-widest mt-2 bg-red-500/10 px-2 py-1 rounded">Overtime</span>}
                       </div>
                   </div>

                   {/* Controls - Flex Row */}
                   <div className="flex items-center gap-6 mb-8 shrink-0">
                        <button onClick={onTogglePlay} className="w-14 h-14 rounded-full border-2 border-zinc-800 flex items-center justify-center text-white hover:bg-zinc-900 transition-all hover:scale-105 active:scale-95 group">
                        ...
                        <div className="flex flex-col gap-2">
                            <button onClick={() => onAdjustTime && onAdjustTime(-60)} className="w-10 h-10 rounded-full bg-zinc-900 text-zinc-500 hover:text-white flex items-center justify-center hover:bg-zinc-800 transition-colors"><Plus size={16} /></button>
                            <button onClick={() => onAdjustTime && onAdjustTime(60)} className="w-10 h-10 rounded-full bg-zinc-900 text-zinc-500 hover:text-white flex items-center justify-center hover:bg-zinc-800 transition-colors"><Minus size={16} /></button>
                        </div>
                   </div>

                   {/* Estimated Completion - Positioned naturally in flow to prevent overlap */}
                   <div className="flex flex-col items-center gap-1 text-zinc-500 shrink-0">
                       <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">Estimated Completion</span>
                       <span className="text-xl font-medium text-zinc-300">{estimatedCompletionTime}</span>
                   </div>
               </div>
          </div>

          {/* RIGHT PANEL: Sequence (Desktop) */}
          <div className="hidden lg:flex flex-col w-80 border-l border-zinc-800 bg-zinc-900/30">
               <div className="p-4 border-b border-zinc-800 bg-zinc-900/50">
                   <div className="flex items-center gap-2 justify-center py-1">
                       <Layers size={14} className="text-zinc-500" />
                       <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-widest">Sequence</h3>
                   </div>
               </div>
               
               <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar" onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, steps.length)}> 
                   {steps.map((step, idx) => {
                               ...
                               <div key={step.id} className="relative">
                                   ...
                                   <div className="bg-zinc-800 border border-zinc-700 p-4 rounded-xl shadow-lg relative overflow-hidden group">
                                       <div className="flex justify-between items-start mb-2">
                                           <span className="bg-emerald-500 text-black text-[10px] font-black px-1.5 py-0.5 rounded uppercase tracking-wide">Now</span>
                                           <div className="text-emerald-500 animate-pulse"><Clock size={14} /></div>
                                       </div>
                                       <h4 className="font-bold text-white text-lg leading-tight mb-1">{step.title}</h4>
                                       <div className="text-xs font-mono text-zinc-400">{Math.round(step.durationSeconds / 60)} min</div>
                                   </div>
                               </div>
                           );
                       }
                       ...
                       return (
                           <div 
                                ...
                                className={`p-4 rounded-xl border transition-all flex items-center gap-3 group relative
                                    ${isPast ? 'bg-transparent border-zinc-800 text-zinc-600' : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:border-zinc-700 hover:bg-zinc-800'}
                                    ${dragOverIndex === idx ? 'border-t-2 border-t-emerald-500 mt-2' : ''}
                                `}
                           >
                               {!isPast && <div className="text-zinc-700 group-hover:text-zinc-500 cursor-grab active:cursor-grabbing"><GripVertical size={16} /></div>}
                               <div className="flex-1 min-w-0">
                                   <div className={`font-bold truncate ${isPast ? 'line-through decoration-zinc-700' : 'text-zinc-300'}`}>{step.title}</div>
                                   <div className="text-xs font-mono opacity-50">{Math.round(step.durationSeconds / 60)} min</div>
                               </div>
      ...
      {/* MOBILE BOTTOM SHEET (Sequence) */}
      <div 
        className={`lg:hidden fixed inset-x-0 bottom-0 bg-zinc-900 border-t border-zinc-800 rounded-t-3xl transition-transform duration-300 z-50 flex flex-col max-h-[80vh] ${isMobileSequenceOpen ? 'translate-y-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]' : 'translate-y-full'}`}
      >
          <div className="p-4 border-b border-zinc-800 flex justify-between items-center" onClick={() => setIsMobileSequenceOpen(false)}>
              <h3 className="font-bold text-white ml-2">Up Next</h3>
              <button className="p-2 bg-zinc-800 rounded-full text-zinc-400"><ChevronDown size={20} /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {steps.map((step, idx) => {
                  if (idx <= currentStepIndex) return null; // Only show future steps
                  return (
                      <div key={step.id} className="bg-zinc-800 p-4 rounded-xl border border-zinc-700 flex justify-between items-center">
                          <div className="font-bold text-zinc-200">{step.title}</div>
                          <div className="text-xs font-mono text-zinc-500">{Math.round(step.durationSeconds/60)}m</div>
                      </div>
                  );
              })}
              {steps.length <= currentStepIndex + 1 && <div className="text-center text-zinc-500 py-8 italic">No more steps</div>}
          </div>
      </div>

      {/* Mobile Backdrop for Sheet */}
      {isMobileSequenceOpen && <div className="lg:hidden fixed inset-0 bg-black/50 z-40" onClick={() => setIsMobileSequenceOpen(false)}></div>}
...
export const MiniPlayer: React.FC<{
    ...
    const progress = currentStep.durationSeconds > 0 ? (timeElapsed / currentStep.durationSeconds) * 100 : 100;
    ...
    return (
        <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-2xl shadow-xl flex items-center gap-3 w-full max-w-sm mx-auto backdrop-blur-md animate-fade-in-up">
            ...
            <div className="relative w-10 h-10 shrink-0 cursor-pointer group" onClick={onTogglePlay}>
                <svg className="w-full h-full transform -rotate-90">
                    <circle cx="50%" cy="50%" r="18" stroke="#333" strokeWidth="3" fill="transparent" />
                    <circle cx="50%" cy="50%" r="18" stroke={isOvertime ? "#ef4444" : "#10b981"} strokeWidth="3" fill="transparent"
                        strokeDasharray={2 * Math.PI * 18}
                        strokeDashoffset={2 * Math.PI * 18 - (Math.min(100, progress) / 100) * (2 * Math.PI * 18)}
                        strokeLinecap="round"
            ...
            {/* Text Info */}
            <div className="flex-1 min-w-0 cursor-pointer flex flex-col justify-center" onClick={onExpand}>
                <div className="flex items-center justify-between gap-2">
                    <h4 className="text-sm font-bold truncate text-zinc-100">{currentStep.title}</h4>
                    <span className={`text-xs font-mono font-medium ${isOvertime ? 'text-red-400 animate-pulse' : 'text-zinc-400'}`}>
                        {formatTime(timeLeft)}
                    </span>
                </div>
            ...
            <div className="flex items-center gap-1 shrink-0 border-l border-zinc-800 pl-2 ml-1">
                 <button onClick={onNext} className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors" title="Next Step">
                    <SkipForward size={16} />
                </button>
                <button onClick={onExpand} className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors" title="Maximize">
                    <Maximize2 size={16} />
                </button>
            </div>
        </div>
